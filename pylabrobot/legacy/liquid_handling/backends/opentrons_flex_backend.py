...
class OpentronsFlexBackend(OpentronsBackend):
  ...
  def _deck_configuration(self) -> List[Dict[str, str]]:
    ...
    trash sits and a single slot otherwise. Column-4 staging slots ride on the column-3 cutouts and
    have no cutout fixture of their own, so they are skipped. Derived from the deck so the robot's
    fixtures cannot disagree with the deck model: FlexDeck(with_trash_bin=False) frees A3 as an
    ordinary slot here too.
    """
    deck = self.deck
    ...
    for slot, resource in deck.slots.items():
      column = int(slot[1])
      if column not in column_fixture:
        continue
      fixture = (
